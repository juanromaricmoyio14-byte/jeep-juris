...
export const consulterAgent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<AgentResult> => {
    ...
    try {
      ...
      if (!res.ok) {
        // 🛡️ Sentinel: Sanitize error logging to prevent leaking sensitive API responses/keys
        console.error(`Gemini API error occurred with status: ${res.status}`);
      ...
      // 🛡️ Sentinel: Do not expose full stack trace or generic error details
      console.error("consulterAgent encountered an internal error during execution.");
