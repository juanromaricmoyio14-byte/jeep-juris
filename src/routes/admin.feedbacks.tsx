        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {(["all", "bug", "suggestion", "compliment"] as const).map((f) => (
            ...
            >
              {f === "all"
                ? t("admin.filterAll")
                : t(`feedback.type${f[0].toUpperCase() + f.slice(1)}`)}
            </button>
          ))}
        </div>
                  ...
                  return (
                    <tr key={f.id} className="border-b border-border last:border-0 align-top">
                      <td className="whitespace-nowrap px-4 py-3 text-xs text-muted-foreground">
                        {date
                          ? date.toLocaleString(lang, { dateStyle: "short", timeStyle: "short" })
                          : "—"}
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={`inline-block rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${TYPE_STYLES[f.type]}`}
                        >
